...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_CFG_MTU_EVT:
        ...
        esp_ble_gattc_search_service(gattc_if, param->cfg_mtu.conn_id, &spp_service_uuid);
        break;
    case ESP_GATTC_SEARCH_RES_EVT:
        if (param->search_res.srvc_id.uuid.uuid.uuid16 == ESP_GATT_SPP_SERVICE_UUID) {
            ESP_LOGI(GATTC_TAG, "Found SPP service");
            spp_srv_start_handle = param->search_res.start_handle;
            spp_srv_end_handle = param->search_res.end_handle;
        }
        break;
    case ESP_GATTC_SEARCH_CMPL_EVT:
        if (param->search_cmpl.status != ESP_GATT_OK) {
            ESP_LOGE(GATTC_TAG, "Search service failed, error status = %x", param->search_cmpl.status);
            break;
        }
        if (spp_srv_start_handle == 0 || spp_srv_end_handle == 0) {
            ESP_LOGE(GATTC_TAG, "SPP service not found");
            break;
        }
        ret = esp_ble_gattc_get_attr_count(gattc_if,
                                          ...
                                          &count);
        if (ret != ESP_GATT_OK) {
            ESP_LOGE(GATTC_TAG, "get attr count error");
            break;
        }
        ESP_LOGI(GATTC_TAG, "attr count = %d", count);
        db = malloc(sizeof(esp_gattc_db_elem_t) * count);
        if (!db) {
            ESP_LOGE(GATTC_TAG, "malloc error");
            break;
        }
        ret = esp_ble_gattc_get_db(gattc_if, param->search_cmpl.conn_id, spp_srv_start_handle, spp_srv_end_handle, db, &count);
        if (ret != ESP_GATT_OK) {
            ESP_LOGE(GATTC_TAG, "get db error");
            break;
        }
        // 注册通知
        for (int i = 0; i < count; i++) {
            if (db[i].uuid.uuid.uuid16 == ESP_GATT_UUID_SPP_DATA_NOTIFY) {
                ...
                gl_profile_tab[PROFILE_APP_ID].char_handle = db[i].attribute_handle;
                esp_ble_gattc_register_for_notify(gattc_if, gl_profile_tab[PROFILE_APP_ID].remote_bda, db[i].attribute_handle);
                break;
            }
        }
        break;
    case ESP_GATTC_NOTIFY_EVT:
        ESP_LOGI(GATTC_TAG, "Received notify, handle = %d, value len = %d", param->notify.handle, param->notify.value_len);
    ...
    case ESP_GATTC_WRITE_CHAR_EVT:
        ESP_LOGI(GATTC_TAG, "Write char complete, status = %d", param->write.status);
        break;
